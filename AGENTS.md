...
- **`GerminalPreferences`** (`germinal-preferences.c/h`) — stateless factory function `germinal_preferences_new()` returning an `AdwPreferencesDialog`. Three pages (Appearance, Terminal, Shell) with per-key reset buttons backed by `g_settings_get_user_value`. Each `GtkColorDialogButton` gets its own fresh `GtkColorDialog` (they have `(transfer full)` semantics — sharing one dialog causes a double-free on finalize). The Appearance page includes a Palette group backed by `GerminalPaletteEditor`. The Shell page ends with an `AdwButtonRow` that presents the about dialog (parented to the preferences dialog) — this is the only in-UI entry point to it, deliberately kept out of the right-click menu to avoid clutter.

- **`germinal-about.c/h`** — stateless factory function `germinal_about_new()` returning an `AdwAboutDialog` populated from build-time constants (`PACKAGE_VERSION` is defined in `meson.build`). Shared by the preferences About button and the `about` command/action in the entry point.

Entry point (`germinal.c`) wires the application via `g_signal_connect`: a `startup` handler sets `ADW_COLOR_SCHEME_PREFER_DARK` and registers the `preferences` and `about` application `GActionEntry`s, and `command-line` / `activate` handlers create a window and trigger `germinal_window_spawn_command`. All three signal IDs are stored in `gulong` variables and explicitly disconnected after `g_application_run` returns. Command-line arguments beyond flags become the startup command, overriding the GSettings value. As a special case, a lone `preferences`/`settings`/`about` argument is mapped by `germinal_command_action` to the matching application action instead of spawning a terminal. Those actions present their dialog via `germinal_present_dialog`, which attaches it to the active window when one exists, or otherwise shows it as its own window and `g_application_hold`s the app (releasing on the dialog's `closed` signal) — so a bare dialog invocation never creates an empty terminal window, and a running instance reuses its existing window. The desktop file (`Actions=preferences;about;`) exposes the same two actions as launcher menu entries; because the app is `DBusActivatable`, the action ids must match these `GAction` names (the `Exec=… germinal preferences`/`about` lines are the fallback for non-D-Bus launchers).
...
- All public API functions must begin with `g_return_if_fail` / `g_return_val_if_fail` precondition guards. Use `GERMINAL_IS_*(self)` for GObject self-checks, `G_IS_SETTINGS(settings)` for GSettings parameters, and `ptr != NULL` for required output pointers.
...
- Automated tests live under `tests/`: `regexp` (headless, validates URL regex), `settings` (headless, validates `germinal_settings_get_palette`), and `palette-editor` (requires display — skips gracefully when `DISPLAY`/`WAYLAND_DISPLAY` are absent, runs fully with Xvfb in CI). All three test files use the GLib test framework (`g_test_init`, `g_test_add_func`, `g_test_run`, `g_assert_*`). Tests that require GTK widgets must call `g_object_ref_sink` on the freshly created widget before using `g_autoptr` or `g_object_unref`, because all `GtkWidget` instances start as floating (`GInitiallyUnowned`).
